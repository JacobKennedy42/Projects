...
//constants used for RMSprop
static const double MOMENTUM = .9;
static const double ONE_SUB_MOM = 1 - MOMENTUM;
static const double LEARN_RATE = 1;//.001;
static const double MIN_GRAD = .001;
...
static const int UPDATE_EVAL = 100000;
...
string fileName = dirName + "/B" + to_string(BOARD_SIZE) + "N" + to_string(NUM_HIDDEN) + "S" + to_string(HIDDEN_SIZE);
...
struct Layer
{
	...
	MatrixXd sumdCdws;
	VectorXd sumdCdbs;
	//TODO:If making input, input does not need sums. Output only needs one dCda
	VectorXd sumdCdas;

	//store the expected values of the squared gradients. used for the rmsprop algorithm
	MatrixXd edCdws;
	VectorXd edCdbs;
	...
	Layer (Layer* inPrev, int numNodes)
	{
		...
		edCdws = MatrixXd::Zero(numNodes, prevSize);
		edCdbs = VectorXd::Zero(numNodes);
...
//			// b_(t+1) = b_t - (learning rate * g_t)/sqrt(E[g^2]_t + smallValue)
...
//				// E[g^2]_t = momentum * E[g^2]_(t-1) + (1-momentum)(g_t)^2
//				// w_(t+1) = w_t - (learning rate * g_t)/sqrt(E[g^2]_t + smallValue)
...
//		sumdCdbs(actionIndex) += (LEARN_RATE * dCdb) / sqrt(edCdbs(actionIndex) + MIN_GRAD);
		...
		double absPlusOne = abs(vals(actionIndex)) + 1;
		double dCdb = sumdCdas(actionIndex) / (absPlusOne * absPlusOne);
		//TODO:find out why the net does better when you multiply dadz, when you might not need it in the calculus
		//TODO: maybe restart with netv1.0, add in the features and see when it slows down and gets worse
		hggbh
...
//			//RMSprop math:
////			//g_t = dC/dw = dC/da * da/dz * dz/dw = 2(a_i-a*) * 1/(1+|a_i|)^2 * a_i-1 = dC/db * a_i-1
//			//g_t = dC/dw = dC/da * da/dw = 2(a_i-a*) * a_i-1 = dC/db * a_i-1
//			// E[g^2]_t = momentum * E[g^2]_(t-1) + (1-momentum)(g_t)^2
//			// w_(t+1) = w_t - (learning rate * g_t)/sqrt(E[g^2]_t + smallValue)
...
//					currentTrans->dbPrint();
		...
		if (game % testSize == 0)
		{
			printf("%lf ", sumTurns / (double) testSize);
			//if the net has done better than before, save it
			if (sumTurns < bestScore)
			{
				bestScore = sumTurns;
				player.saveNet();
			}
